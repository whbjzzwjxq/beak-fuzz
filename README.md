...
It installs each configured target, runs the benchmark/injection workload serially, and writes:
...
The serial campaign does not set a per-eval or per-step timeout, because each semantic step/variant can have VM-specific runtime. The outer per-target budget still defaults to 4 hours (`SOFT_TIMEOUT_SECONDS=14400`).
