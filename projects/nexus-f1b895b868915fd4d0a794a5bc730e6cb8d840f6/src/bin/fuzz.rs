...
use beak_nexus_f1b895b::backend::NexusBackend;
use beak_nexus_f1b895b::NEXUS_ORACLE_CODE_BASE;

const ZKVM_COMMIT: &str = "f1b895b868915fd4d0a794a5bc730e6cb8d840f6";
...
fn write_inline_seed_jsonl(root: &Path, words: &[u32]) -> PathBuf {
    ...
    let dir = root.join("projects/nexus-f1b895b868915fd4d0a794a5bc730e6cb8d840f6/out");
    std::fs::create_dir_all(&dir).expect("create project out dir");
    let path = dir.join(format!(".tmp-inline-nexus-f1b895b-{ts}.jsonl"));
    let line = json!({
        "instructions": words,
        "metadata": {
            "source": "cli_bin",
...
fn main() {
    let matches = Command::new("beak-fuzz")
        ...
        .arg(
            Arg::new("out_dir")
                .long("out-dir")
                .default_value("projects/nexus-f1b895b868915fd4d0a794a5bc730e6cb8d840f6/out")
    ...
    let res = run_benchmark_threaded(cfg, move || NexusBackend::new(max_instructions));
    match res {
        Ok(out) => {
