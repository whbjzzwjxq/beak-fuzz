...
use beak_jolt_6c3b0b49::backend::{run_backend_once, JoltBackend, WorkerRequest, WorkerResponse};
use beak_jolt_6c3b0b49::JOLT_ORACLE_CODE_BASE;

const ZKVM_COMMIT: &str = "6c3b0b49db0afceb967b33656176fa7a27e557b9";
...
fn write_inline_seed_jsonl(out_dir: &Path, words: &[u32]) -> PathBuf {
    ...
    std::fs::create_dir_all(out_dir).expect("create inline seed directory");
    let path = out_dir.join(format!(".tmp-inline-jolt-6c3b0b49-{ts}.jsonl"));
    let line = json!({
        "instructions": words,
        "metadata": {
            "source": "cli_bin",
...
fn main() {
    let matches = Command::new("beak-fuzz")
        .about("Initial-corpus benchmark with the vulnerable Jolt 6c prover path.")
        .arg(
            Arg::new("bin")
                .long("bin")
        ...
        .arg(
            Arg::new("oracle_code_base")
                ...
                .help("Oracle code base address for split-code-data mode."),
        )
        .arg(
    ...
    let out_dir_arg = matches.get_one::<String>("out_dir").unwrap().to_string();
    let out_dir = resolve_path(&root, &out_dir_arg);
