        ...
        for (store, prev_value) in &stores {
                ...
                .find(|load| load.op_idx > store.op_idx && load.address == store.address)
            {
                if let Some(cell_id) = me1_cell(&store.mnemonic, &load.mnemonic) {
                    ...
                    details.insert("store_step_idx".to_string(), json!(store.op_idx));
                    details.insert("store_timestamp".to_string(), json!(store.timestamp));
                    details.insert("store_pc".to_string(), json!(store.pc));
                    details.insert(
                        "store_opcode".to_string(),
                        json!(format!("0x{:08x}", store.raw_word)),
                    );
                    details.insert("store_raw_word".to_string(), json!(store.raw_word));
                    details.insert("store_mnemonic".to_string(), json!(store.mnemonic));
                    details.insert("store_address".to_string(), json!(store.address));
                    details.insert("store_effective_ptr".to_string(), json!(store.address));
                    details.insert("store_width".to_string(), json!(store.size_bytes));
                    details.insert("store_value".to_string(), json!(store.value));
                    details.insert("store_value_before".to_string(), json!(store.value));
                    details.insert("write_data".to_string(), json!(store.value));
                    details.insert("prev_data".to_string(), json!(prev_value));
                    details.insert("load_step_idx".to_string(), json!(load.op_idx));
                    details.insert("load_timestamp".to_string(), json!(load.timestamp));
                    details.insert("load_pc".to_string(), json!(load.pc));
                    details.insert(
                        "load_opcode".to_string(),
                        json!(format!("0x{:08x}", load.raw_word)),
                    );
                    details.insert("load_raw_word".to_string(), json!(load.raw_word));
                    details.insert("load_mnemonic".to_string(), json!(load.mnemonic));
                    details.insert("load_address".to_string(), json!(load.address));
                    details.insert("load_effective_ptr".to_string(), json!(load.address));
                    details.insert("load_width".to_string(), json!(load.size_bytes));
                    details.insert("load_value_before".to_string(), json!(load.value));
                    details.insert("read_data".to_string(), json!(load.value));
                    bucket_hits.push(BucketHit::semantic(
