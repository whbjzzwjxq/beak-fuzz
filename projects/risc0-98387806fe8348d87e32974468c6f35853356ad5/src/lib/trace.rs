...
fn decoded_next_pc(pc: u32, dec: &RV32IMInstruction, regs_before: &[u32; 32]) -> u32 {
    let imm_pc = || pc.wrapping_add(dec.imm.unwrap_or(0) as u32);
    let reg = |idx: Option<u32>| idx.map(|idx| regs_before[idx as usize]).unwrap_or(0);
    match dec.mnemonic.as_str() {
        "beq" if reg(dec.rs1) == reg(dec.rs2) => imm_pc(),
        "bne" if reg(dec.rs1) != reg(dec.rs2) => imm_pc(),
        "blt" if (reg(dec.rs1) as i32) < (reg(dec.rs2) as i32) => imm_pc(),
        "bge" if (reg(dec.rs1) as i32) >= (reg(dec.rs2) as i32) => imm_pc(),
        "bltu" if reg(dec.rs1) < reg(dec.rs2) => imm_pc(),
        "bgeu" if reg(dec.rs1) >= reg(dec.rs2) => imm_pc(),
        "jal" => imm_pc(),
        "jalr" => reg(dec.rs1).wrapping_add(dec.imm.unwrap_or(0) as u32) & !1,
        _ => pc.wrapping_add(4),
    }
}
...
pub(crate) fn executed_instructions(words: &[u32]) -> Result<Vec<Risc0Insn>, String> {
    ...
    for (idx, executed) in executed_steps.iter().enumerate() {
        ...
        let computed_next_pc = decoded_next_pc(executed.pc, &dec, &regs_before);
        let next_pc = executed_steps.get(idx + 1).map(|next| next.pc).unwrap_or(computed_next_pc);
...
fn preflight_executed_instructions(
    ...
    for (idx, record) in records.iter().enumerate() {
        ...
        let computed_next_pc = decoded_next_pc(record.pc, &dec, &regs_before);
        let next_pc = records.get(idx + 1).map(|next| next.pc).unwrap_or(computed_next_pc);
        instructions.push(Risc0Insn {
