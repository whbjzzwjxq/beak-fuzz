...
impl OpenVMTrace {
    ...
    pub fn extend_instruction_local_obligation_hits(&mut self, words: &[u32]) {
    ...
    fn derive_chip_row_obligation_hits(
        ...
        for row in &self.chip_rows {
            let base = row.base();
            let Some(insn) = decoded_by_step.get(&base.step_idx) else {
                continue;
            };
            let mnemonic = insn.mnemonic.as_str();
            let row_extras = [
            ...
            match &row.payload {
                    ...
                    if matches!(mnemonic, "slt" | "sltu" | "slti" | "sltiu") =>
                {
                        ...
                        (le_u32_from_bytes(a), le_u32_from_bytes(b), le_u32_from_bytes(c))
                    {
                        ...
                        self.push_comparison_aux_hits(hits, insn, base.step_idx, &extras, lhs, rhs);
                    }
                }
