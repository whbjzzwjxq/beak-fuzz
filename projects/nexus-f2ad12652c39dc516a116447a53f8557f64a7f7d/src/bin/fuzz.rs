...
use beak_nexus_f2ad126::NEXUS_ORACLE_CODE_BASE;

const ZKVM_COMMIT: &str = "f2ad12652c39dc516a116447a53f8557f64a7f7d";
...
fn write_inline_seed_jsonl(root: &Path, words: &[u32]) -> PathBuf {
    ...
    let dir = root.join("projects/nexus-f2ad12652c39dc516a116447a53f8557f64a7f7d/out");
    std::fs::create_dir_all(&dir).expect("create project out dir");
    let path = dir.join(format!(".tmp-inline-nexus-f2ad126-{ts}.jsonl"));
    let line = json!({
        "instructions": words,
        "metadata": {
            "source": "cli_bin",
