...
const NEXUS_PROVER2_READ_WRITE_MEMORY_MIN_LOG_SIZE: u32 = 4;
...
fn default_private_memory_range() -> (u32, u32) {
    let layout = LinearMemoryLayout::default();
    (layout.heap_start(), layout.stack_top())
}

fn memory_instruction_rows(
    loads: &[ExecutedMemoryAccess],
    stores: &[(ExecutedMemoryAccess, u32)],
) -> usize {
    loads.len().saturating_add(stores.len())
}

fn emit_read_write_memory_padding_hit(
    bucket_hits: &mut Vec<BucketHit>,
    loads: &[ExecutedMemoryAccess],
    stores: &[(ExecutedMemoryAccess, u32)],
) {
    ...
    let log_size = table_log_size(real_rows, NEXUS_PROVER2_READ_WRITE_MEMORY_MIN_LOG_SIZE);
    let padded_rows = 1usize << log_size;
    if real_rows >= padded_rows {
        return;
    }
    let padding_row_idx = real_rows as u64;
    let mut details = HashMap::new();
    details.insert("obligation_id".to_string(), json!("pd1"));
    details.insert("cell_id".to_string(), json!("pd1.mem_padding"));
    details.insert("backend".to_string(), json!(BACKEND));
    details.insert("commit".to_string(), json!(COMMIT));
    details.insert("trace_source".to_string(), json!("prover2.read_write_memory.padding"));
    details.insert("step_idx".to_string(), json!(padding_row_idx));
    details.insert("padding_row_idx".to_string(), json!(padding_row_idx));
    details.insert("table_name".to_string(), json!("read_write_memory"));
    details.insert("bus_name".to_string(), json!("rel_ram_read_write"));
    details.insert("interaction_kind".to_string(), json!("consume_provide"));
    details.insert("is_padding".to_string(), json!(true));
    details.insert("real_rows".to_string(), json!(real_rows));
    details.insert("log_size".to_string(), json!(log_size));
    details.insert("padded_rows".to_string(), json!(padded_rows));
    details.insert("padding_rows".to_string(), json!(padded_rows.saturating_sub(real_rows)));
    bucket_hits.push(BucketHit::semantic(semantic::row::PADDING_INTERACTION_SEND, details));
}

fn emit_private_memory_finalization_hits(
    bucket_hits: &mut Vec<BucketHit>,
    loads: &[ExecutedMemoryAccess],
    stores: &[(ExecutedMemoryAccess, u32)],
) {
    ...
    for (store, prev_value) in stores {
        ...
        details.insert("final_value".to_string(), json!(store.value & 0xff));
        details.insert("final_timestamp".to_string(), json!(store.timestamp));
        details.insert("was_initial".to_string(), json!(false));
        details.insert(
            "changed_from_initial".to_string(),
            json!((store.value & 0xff) != (prev_value & 0xff)),
        );
...
impl NexusTrace {
    ...
    pub fn from_words_and_uniform_trace(_words: &[u32], trace: &UniformTrace) -> Self {
        ...
        emit_private_memory_finalization_hits(&mut bucket_hits, &loads, &stores);
        emit_read_write_memory_padding_hit(&mut bucket_hits, &loads, &stores);
