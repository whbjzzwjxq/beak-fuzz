...
use sp1_core_executor::{ByteOpcode, ExecutionRecord, Opcode, SP1CoreOpts};
use sp1_core_machine::{io::SP1Stdin, utils::generate_records};
use sp1_primitives::SP1Field;

use crate::trace::{
    build_sp1_program, decode_word_to_sp1_instruction, executed_instruction_events,
    memory_instruction_events, Sp1Trace,
};
...
const ID3_INJECT_KIND: &str = "sp1.semantic.decode.upper_immediate_materialization";
...
const MEMORY_INITIAL_INJECT_KIND: &str = "sp1.semantic.memory.initial_value_binding";
const ME11_INJECT_KIND: &str = "sp1.semantic.memory.finalization_consistency";
const CF7_INJECT_KIND: &str = "sp1.semantic.control.ecall_word_validity";
const TS_BOUNDARY_INJECT_KIND: &str = "sp1.semantic.time.boundary_origin_consistency";
const PD1_INJECT_KIND: &str = "sp1.semantic.row.padding_interaction_send";
...
fn byte_lookup_step(opcode: ByteOpcode, a: u16, b: u8, c: u8) -> u64 {
    let row =
        if opcode == ByteOpcode::Range { a as u64 } else { (((b as u16) << 8) + c as u16) as u64 };
    row.saturating_mul(6).saturating_add(opcode as u64)
}
...
fn supports_official_injection_kind(kind: &str) -> bool {
    matches!(
        ...
        MEMORY_EFFECT_INJECT_KIND
            ...
            | MD4_INJECT_KIND
            | MD5_INJECT_KIND
            | MEMORY_INITIAL_INJECT_KIND
            | ME11_INJECT_KIND
            | CF7_INJECT_KIND
            | TS_BOUNDARY_INJECT_KIND
            | PD1_INJECT_KIND
    )
}
...
fn collect_observed_injection_sites(records: &[ExecutionRecord]) -> BTreeMap<String, Vec<u64>> {
    ...
    let executed_events = executed_instruction_events(records);
    for event in &executed_events {
        let instruction_step = event.seq;
        let chip_step = event.pc / 4;
        record_alu_muldiv_chip_sites(&mut sites, event.opcode, chip_step);
        ...
        if matches!(
            ...
            record_site(&mut sites, MEMORY_EFFECT_INJECT_KIND, instruction_step);
        }
        if let Some(family) = control_flow_family_for_opcode(event.opcode) {
            record_site(&mut sites, CONTROL_FLOW_INJECT_KIND, instruction_step);
            record_site(&mut sites, &control_flow_site_key(family), instruction_step);
        }
        for kind in [
            RF1_INJECT_KIND,
            RF2_INJECT_KIND,
            ...
            record_site(&mut sites, kind, instruction_step);
        }
        if matches!(event.opcode, Opcode::LUI | Opcode::AUIPC) {
            record_site(&mut sites, ID3_INJECT_KIND, instruction_step);
        }
        if event.opcode == Opcode::ECALL {
            record_site(&mut sites, CF7_INJECT_KIND, instruction_step);
        }
    }
    if let Some(first) = executed_events.first() {
        record_site(&mut sites, TS_BOUNDARY_INJECT_KIND, first.seq);
    }
    for event in memory_instruction_events(records) {
        record_memory_instr_sites(&mut sites, event.opcode, event.memory_hook_step);
    }
    for record in records {
        ...
        for event in &record.global_memory_initialize_events {
            record_site(&mut sites, MEMORY_ADDRESS_INJECT_KIND, event.timestamp);
            record_site(&mut sites, MEMORY_INITIAL_INJECT_KIND, event.timestamp);
        }
        for event in &record.global_memory_finalize_events {
            record_site(&mut sites, MEMORY_ADDRESS_INJECT_KIND, event.timestamp);
            record_site(&mut sites, ME11_INJECT_KIND, event.timestamp);
        }
        if !record.syscall_events.is_empty() && record.syscall_events.len() % 32 != 0 {
            record_site(&mut sites, PD1_INJECT_KIND, record.syscall_events.len() as u64);
        }
        for (lookup, mult) in &record.byte_lookups {
            ...
            record_site(
                &mut sites,
                LOOKUP_BOOLEAN_INJECT_KIND,
                byte_lookup_step(lookup.opcode, lookup.a, lookup.b, lookup.c),
            );
        }
    }
    sites
...
fn with_scoped_injection_env<T>(
    ...
    std::env::set_var("BEAK_SP1_WITNESS_RUN_ID", format!("sp1-98a376e-{run_id}"));
...
fn run_sp1_prove_verify(
    ...
    if !supports_official_injection(inject_kind) {
        return (
            ...
            false,
            Some(format!(
                "sp1-98a376e has no installed hook/applied signal for requested kind {}; mapped hooks are executor tracing hooks, {MEMORY_EFFECT_INJECT_KIND}, {CONTROL_FLOW_INJECT_KIND}, v4 memory-instruction hooks, and byte-table multiplicity hooks",
    ...
    let prove_result = with_scoped_injection_env(
        ...
        || {
            ...
            match result {
                Ok(Ok(_)) => Ok(()),
                Ok(Err(err)) => Err(format!("sp1 v4 injected record generation failed: {err}")),
                Err(panic) => Err(format!(
                    "sp1 v4 injected record generation panicked: {}",
...
fn final_regs_from_records(records: &[ExecutionRecord]) -> [u32; 32] {
    let mut regs = [0u32; 32];
    for event in records.iter().flat_map(|record| &record.global_memory_finalize_events) {
        let idx = event.addr as usize;
        if idx < regs.len() {
            regs[idx] = event.value as u32;
        }
    }
    regs[0] = 0;
    regs
...
fn run_sp1_real_backend(
    ...
    .map_err(|e| format!("sp1 generate_records failed: {e}"))?;
    ...
    Ok(RealRunnerResponse {
        final_regs: Some(final_regs_from_records(&records)),
...
impl Sp1Backend {
    ...
    fn padding_hook_step_from_hit(hit: &BucketHit) -> u64 {
        hit.details
            .get("padding_hook_step")
            .and_then(|v| v.as_u64())
            .or_else(|| hit.details.get("padding_row_idx").and_then(|v| v.as_u64()))
            .unwrap_or_else(|| Self::step_from_hit(hit))
    }
    ...
    fn semantic_candidate_from_hit(&self, hit: &BucketHit) -> Vec<SemanticInjectionCandidate> {
        ...
        let anchor = if Self::is_chip_scheduled_bucket(bucket_id) {
            ...
            Self::padding_hook_step_from_hit(hit)
            ...
            || bucket_id == semantic::memory::TIMESTAMPED_LOAD_PATH.id
        {
            ...
            (
                semantic::decode::UPPER_IMMEDIATE_MATERIALIZATION.semantic_class.to_string(),
                inject_kind_with_variant(ID3_INJECT_KIND, "site=instruction_op_b"),
                InjectionSchedule::AroundAnchor(anchor),
            )
            ...
            (
                semantic::memory::FINALIZATION_CONSISTENCY.semantic_class.to_string(),
                inject_kind_with_variant(ME11_INJECT_KIND, "site=value,phase=finalize"),
                InjectionSchedule::Explicit(vec![anchor]),
            )
        } else if bucket_id == semantic::memory::INITIAL_VALUE_BINDING.id {
            if Self::detail_str(hit, "trace_source") != Some("memory_initialization")
                || Self::detail_str(hit, "cell_id") != Some("me8.no_conflict")
            {
                return Vec::new();
            }
            (
                semantic::memory::INITIAL_VALUE_BINDING.semantic_class.to_string(),
                inject_kind_with_variant(MEMORY_INITIAL_INJECT_KIND, "site=value,phase=initialize"),
                InjectionSchedule::Explicit(vec![anchor]),
            )
        } else if bucket_id == semantic::control::ECALL_WORD_VALIDITY.id {
            (
                semantic::control::ECALL_WORD_VALIDITY.semantic_class.to_string(),
                inject_kind_with_variant(CF7_INJECT_KIND, "site=syscall_arg"),
                InjectionSchedule::AroundAnchor(anchor),
            )
        } else if bucket_id == semantic::time::BOUNDARY_ORIGIN_CONSISTENCY.id {
            (
                semantic::time::BOUNDARY_ORIGIN_CONSISTENCY.semantic_class.to_string(),
                inject_kind_with_variant(TS_BOUNDARY_INJECT_KIND, "site=clk"),
                InjectionSchedule::AroundAnchor(anchor),
            )
        } else if bucket_id == semantic::row::PADDING_INTERACTION_SEND.id {
            if !self.last_observed_injection_sites.contains_key(PD1_INJECT_KIND) {
                return Vec::new();
            }
            (
                semantic::row::PADDING_INTERACTION_SEND.semantic_class.to_string(),
                inject_kind_with_variant(PD1_INJECT_KIND, "site=syscall_instr_padding_send_table"),
                InjectionSchedule::Explicit(vec![anchor]),
            )
        } else {
            return Vec::new();
