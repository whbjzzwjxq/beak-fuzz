...
use beak_pico_22b0aae::backend::{run_backend_once, PicoBackend, WorkerRequest, WorkerResponse};

const ZKVM_COMMIT: &str = "22b0aae6321c1f63c72aafd0b506b5f45b91ffb1";
...
fn write_inline_seed_jsonl(root: &Path, words: &[u32]) -> PathBuf {
    ...
    let path = dir.join(format!(".tmp-inline-pico-22b0aae-{ts}.jsonl"));
    let line = json!({
        "instructions": words,
        "metadata": {
            "source": "cli_bin",
...
fn main() {
    let matches = Command::new("beak-fuzz")
        .about("Initial-corpus benchmark with semantic witness search (oracle vs Pico).")
        .arg(
            Arg::new("bin")
                .long("bin")
    ...
    let res = run_benchmark_threaded(cfg, move || PicoBackend::new(max_instructions));
    match res {
        Ok(out) => {
