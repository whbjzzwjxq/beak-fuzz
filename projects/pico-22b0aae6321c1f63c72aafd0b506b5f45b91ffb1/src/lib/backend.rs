...
impl PicoBackend {
    ...
    fn semantic_candidate_from_hit(&self, hit: &BucketHit) -> Vec<SemanticInjectionCandidate> {
        ...
        if !latest_install_hook_exists(inject_kind) {
            return Vec::new();
        }
        let schedule = self
...
mod tests {
    ...
    #[test]
    fn id4_load_store_candidates_use_cpu_selector_hook() {
        ...
        assert_eq!(candidates[0].inject_kind, OP_SELECTOR_INJECT_KIND);
    }
    ...
    #[test]
    fn timestamp_candidates_use_latest_memory_hook() {
        ...
        let backend = PicoBackend::new(8);
        let timestamp_hits = trace
            .bucket_hits()
            .iter()
            .filter(|hit| {
                hit.bucket_id == semantic::memory::TIMESTAMPED_LOAD_PATH.id
        ...
        assert!(!timestamp_hits.is_empty());
        assert!(!candidates.is_empty());
        assert!(candidates
            .iter()
            .any(|candidate| candidate.inject_kind.starts_with(TIMESTAMP_INJECT_KIND)));
    }

    #[test]
    fn latest_alu_muldiv_candidates_are_advertised() {
        let cases = [
            (0x0031_10b3, semantic::alu::SHIFT_MOD32.id, SHIFT_INJECT_KIND),
            (0x0031_20b3, semantic::alu::COMPARISON_BOOLEANITY.id, CMP_BOOL_INJECT_KIND),
            (0x0031_20b3, semantic::alu::COMPARISON_AUXILIARY_CHAIN.id, CMP_AUX_INJECT_KIND),
            (0x0231_00b3, semantic::arithmetic::PRODUCT_DECOMPOSITION.id, PRODUCT_INJECT_KIND),
            (
                0x0231_20b3,
                semantic::arithmetic::SIGNED_UNSIGNED_PRODUCT_CORRECTION.id,
                MULHSU_INJECT_KIND,
            ),
            (
                0x0231_40b3,
            ...
            (0x0231_40b3, semantic::arithmetic::DIVISION_REMAINDER_BOUND.id, DIV_BOUND_INJECT_KIND),
        ];

        let backend = PicoBackend::new(8);
        for (word, bucket_id, inject_kind) in cases {
            let trace = PicoTrace::from_words(&[word]).expect("trace");
            let hit = trace
                .bucket_hits()
                .iter()
                .find(|hit| hit.bucket_id == bucket_id)
                .unwrap_or_else(|| panic!("missing bucket {bucket_id} for 0x{word:08x}"))
                .clone();
            let candidates =
                <PicoBackend as BenchmarkBackend>::semantic_injection_candidates(&backend, &[hit]);
            assert_eq!(candidates.len(), 1, "candidate count for {bucket_id}");
            assert_eq!(candidates[0].inject_kind, inject_kind);
        }
    }
}
