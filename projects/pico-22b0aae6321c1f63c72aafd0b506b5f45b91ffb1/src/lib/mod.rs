use std::fs;
use std::path::Path;
use std::process::Command;

pub mod backend;
pub mod chip_row;
pub mod insn;
pub mod interaction;
pub mod trace;

pub type Pc = u32;
pub type Timestamp = u64;
pub type FieldElement = u32;

pub const PICO_COMMIT: &str = "22b0aae6321c1f63c72aafd0b506b5f45b91ffb1";
pub const PICO_REPO_URL: &str = "https://github.com/brevis-network/pico.git";

pub fn run_checked(cmd: &mut Command) -> Result<(), String> {
    let status = cmd.status().map_err(|e| format!("failed to spawn command: {e}"))?;
    if status.success() {
        Ok(())
    } else {
        Err(format!("command failed with status: {status}"))
    }
}

pub fn ensure_pico_checkout(zkvm_dir: &Path) -> Result<(), String> {
    let parent =
        zkvm_dir.parent().ok_or_else(|| format!("invalid zkvm dir: {}", zkvm_dir.display()))?;
    fs::create_dir_all(parent).map_err(|e| format!("create parent dir failed: {e}"))?;

    if !zkvm_dir.join(".git").exists() {
        let mut clone = Command::new("git");
        clone.args(["clone", PICO_REPO_URL, zkvm_dir.to_string_lossy().as_ref()]);
        run_checked(&mut clone).map_err(|e| format!("git clone failed: {e}"))?;
    }

    let mut fetch = Command::new("git");
    fetch.args(["-C", zkvm_dir.to_string_lossy().as_ref(), "fetch", "--all", "--tags", "--prune"]);
    run_checked(&mut fetch).map_err(|e| format!("git fetch failed: {e}"))?;

    let mut checkout = Command::new("git");
    checkout.args(["-C", zkvm_dir.to_string_lossy().as_ref(), "checkout", "--force", PICO_COMMIT]);
    run_checked(&mut checkout).map_err(|e| format!("git checkout failed: {e}"))?;
    Ok(())
}
