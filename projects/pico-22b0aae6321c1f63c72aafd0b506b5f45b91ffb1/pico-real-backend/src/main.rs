...
use pico_vm::{
    ...
    emulator::{
        ...
        riscv::{
            ...
            riscv_emulator::{RiscvEmulator, RiscvEmulatorMode},
...
fn latest_install_hook_exists(kind: &str) -> bool {
    matches!(
...
fn i_from_r(opcode: Opcode, dec: &RType) -> Instruction {
    Instruction::new(opcode, dec.rd as u8, dec.rs1 as u64, dec.rs2 as u64, false, false)
}

fn i_from_i(opcode: Opcode, dec: &IType) -> Instruction {
    Instruction::new(opcode, dec.rd as u8, dec.rs1 as u64, dec.imm as i64 as u64, false, true)
}

fn i_from_i_shamt(opcode: Opcode, dec: &ITypeShamt) -> Instruction {
    Instruction::new(opcode, dec.rd as u8, dec.rs1 as u64, dec.shamt as u64, false, true)
}

fn i_from_s(opcode: Opcode, dec: &SType) -> Instruction {
    Instruction::new(opcode, dec.rs2 as u8, dec.rs1 as u64, dec.imm as i64 as u64, false, true)
}

fn i_from_b(opcode: Opcode, dec: &BType) -> Instruction {
    Instruction::new(opcode, dec.rs1 as u8, dec.rs2 as u64, dec.imm as i64 as u64, false, true)
}
...
impl InstructionProcessor for Transpiler {
    ...
    fn process_jal(&mut self, dec: JType) -> Self::InstructionResult {
        Instruction::new(Opcode::JAL, dec.rd as u8, dec.imm as i64 as u64, 0, true, true)
    }
    fn process_jalr(&mut self, dec: IType) -> Self::InstructionResult {
        Instruction::new(
            ...
            dec.rd as u8,
            dec.rs1 as u64,
            dec.imm as i64 as u64,
            false,
            true,
        )
    }
    fn process_lui(&mut self, dec: UType) -> Self::InstructionResult {
        Instruction::new(Opcode::ADD, dec.rd as u8, 0, dec.imm as u64, true, true)
    }
    fn process_auipc(&mut self, dec: UType) -> Self::InstructionResult {
        Instruction::new(Opcode::AUIPC, dec.rd as u8, dec.imm as u64, dec.imm as u64, true, true)
    }
...
fn mutate_records_for_injection(
    ...
    if !kind.is_empty() {
        ...
        if !latest_install_hook_exists(kind) {
    ...
    for record in records {
        ...
        for event in &record.cpu_events {
            ...
            if event.instruction.is_memory_instruction() {
                ...
                record_site(&mut sites, MEM_ADDR_ALIGN_INJECT_KIND, cpu_anchor);
                record_site(&mut sites, MEM_ADDR_PROGRESS_INJECT_KIND, memory_step);
                record_site(&mut sites, MEM_ADDR_PROGRESS_INJECT_KIND, cpu_anchor);
                record_site(&mut sites, MEM_ADDR_BOUNDARY_INJECT_KIND, memory_step);
                record_site(&mut sites, MEM_ADDR_BOUNDARY_INJECT_KIND, cpu_anchor);
                record_site(&mut sites, MEM_KIND_INJECT_KIND, memory_step);
                record_site(&mut sites, MEM_KIND_INJECT_KIND, cpu_anchor);
                record_site(&mut sites, TIMESTAMP_INJECT_KIND, memory_step);
                record_site(&mut sites, TIMESTAMP_INJECT_KIND, cpu_anchor);
                if matches!(
                    event.instruction.opcode,
                    Opcode::LB | Opcode::LH | Opcode::LW | Opcode::LBU | Opcode::LHU
                ) {
                    record_site(&mut sites, MEM_LOAD_VALUE_INJECT_KIND, memory_step);
                    record_site(&mut sites, MEM_LOAD_VALUE_INJECT_KIND, cpu_anchor);
                    record_site(&mut sites, MEM_STORE_LOAD_INJECT_KIND, memory_step);
                    record_site(&mut sites, MEM_STORE_LOAD_INJECT_KIND, cpu_anchor);
                } else if matches!(event.instruction.opcode, Opcode::SB | Opcode::SH | Opcode::SW)
                {
                    record_site(&mut sites, MEM_WRITE_PAYLOAD_INJECT_KIND, memory_step);
                    record_site(&mut sites, MEM_WRITE_PAYLOAD_INJECT_KIND, cpu_anchor);
                    record_site(&mut sites, MEM_STORE_LOAD_INJECT_KIND, memory_step);
                    record_site(&mut sites, MEM_STORE_LOAD_INJECT_KIND, cpu_anchor);
                }
        ...
        for event in record.shift_left_events.iter().chain(record.shift_right_events.iter()) {
            if event.is_imm {
                record_site(&mut sites, ALU_IMM_INJECT_KIND, event.clk);
            }
            record_site(&mut sites, SHIFT_INJECT_KIND, event.clk);
        }
        for event in &record.lt_events {
            if event.is_imm {
                record_site(&mut sites, ALU_IMM_INJECT_KIND, event.clk);
            }
            record_site(&mut sites, CMP_BOOL_INJECT_KIND, event.clk);
            record_site(&mut sites, CMP_AUX_INJECT_KIND, event.clk);
        }
        for event in &record.mul_events {
            record_site(&mut sites, PRODUCT_INJECT_KIND, event.clk);
            if event.opcode == Opcode::MULHSU {
                record_site(&mut sites, MULHSU_INJECT_KIND, event.clk);
            }
        }
        for event in &record.divrem_events {
            record_site(&mut sites, DIV_SPECIAL_INJECT_KIND, event.clk);
            record_site(&mut sites, DIV_BOUND_INJECT_KIND, event.clk);
        }
    }
...
fn ecall_syscall_id(event: &pico_vm::chips::chips::riscv_cpu::event::CpuEvent) -> Option<u32> {
    ...
    match event.a_record {
        Some(MemoryRecordEnum::Write(record)) => Some(record.prev_value as u32),
        Some(MemoryRecordEnum::Read(record)) => Some(record.value as u32),
        None => Some(event.a as u32),
    }
}
    ...
    for record in records {
        for event in &record.cpu_events {
            ...
            out.push(ExecutedInsn {
                ...
                clk: event.clk,
                pc: event.pc as u32,
                next_pc: event.next_pc as u32,
                word,
                opcode: event.instruction.opcode.mnemonic().to_string(),
                a: event.a as u32,
                b: event.b as u32,
                c: event.c as u32,
                memory: event.memory.map(|value| value as u32),
                ecall_syscall_id,
...
fn run_one(
    ...
    let mut emulator =
        RiscvEmulator::new_single::<Field>(program.clone(), EmulatorOpts::default(), None);
    emulator.mode = RiscvEmulatorMode::Trace;
    let mut records = Vec::<EmulationRecord>::new();
    loop {
        let report = emulator
            .emulate_batch(&mut |record| records.push(record))
            .map_err(|e| format!("emulator emulate_batch(trace) failed: {e:?}"))?;
        if report.done {
            break;
        }
    }
    let regs = emulator.registers().map(|value| value as u32);
