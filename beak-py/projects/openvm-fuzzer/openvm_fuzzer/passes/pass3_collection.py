...
def _patch_f038_volatile_boundary_collection_and_remap(openvm_install_path = None):
    ...
    if program.exists():
        c = program.read_text()
        # The o25 hook is append-shaped at the boundary chip; the older program-trace
        # operand remap belongs to the abandoned in-place approach and must be removed.
        remap_marker = '// BEAK-INSERT: guard.f038.program_trace.volatile_boundary_remap'
        if remap_marker in c:
            start = c.rfind('\n', 0, c.index(remap_marker)) + 1
    ...
    if volatile.exists():
        ...
        height_anchor = '        let memory_len = sorted_final_memory.len();\n'
        height_marker = '// BEAK-INSERT: guard.f038.volatile.o25.height_room'
        height_insert = '\n        // BEAK-INSERT: guard.f038.volatile.o25.height_room\n        // The o25 append-shaped hook needs one spare row above the final-memory rows.\n        let trace_height = if trace_height == memory_len\n            && fuzzer_utils::active_volatile_boundary_remap(\n                "openvm.semantic.memory.volatile_boundary_range",\n            )\n            .is_some()\n        {\n            (memory_len + 1).next_power_of_two()\n        } else {\n            trace_height\n        };\n        // BEAK-INSERT-END\n'
        if height_marker not in c and height_anchor in c:
            c = c.replace(height_anchor, height_anchor + height_insert, 1)
        row_marker = '// BEAK-INSERT: guard.f038.volatile.o25.row_witness'
        if row_marker in c:
            row_start = c.rfind('\n', 0, c.index(row_marker)) + 1
            row_end_marker = '        // BEAK-INSERT-END\n'
            row_end = c.index(row_end_marker, row_start) + len(row_end_marker)
            c = c[:row_start] + c[row_end:]
        post_anchor = '        let trace = RowMajorMatrix::new(rows, width);\n'
        post_insert = '        // BEAK-INSERT: guard.f038.volatile.o25.row_witness\n        if let Some(remap) = fuzzer_utils::active_volatile_boundary_remap(\n            "openvm.semantic.memory.volatile_boundary_range",\n        ) {\n            let anchor_idx = memory_len.saturating_sub(1);\n            let (anchor_as, anchor_ptr) = if memory_len > 0 {\n                let anchor_row: &mut VolatileBoundaryCols<_> =\n                    rows[width * anchor_idx..width * anchor_idx + width].borrow_mut();\n                (\n                    anchor_row.addr_space.as_canonical_u32(),\n                    anchor_row.pointer.as_canonical_u32(),\n                )\n            } else {\n                (0, 0)\n            };\n            let anchor_matches = memory_len > 0\n                && memory_len < trace_height\n                && remap.row_idx == anchor_idx as u64\n                && remap.cell_id == "rc3.volatile_pointer"\n                && anchor_as == remap.address_space\n                && anchor_ptr >= remap.base_pointer\n                && anchor_ptr < remap.base_pointer.saturating_add(remap.width);\n            if anchor_matches {\n                let forged_as = remap.forged_address_space;\n                let forged_ptr = remap.forged_base_pointer;\n                // Append a self-balancing forged boundary row: initial == final and\n                // final_timestamp == 0, so its bus send/receive cancel each other and\n                // no memory-log counterpart is required.\n                {\n                    let forged_row: &mut VolatileBoundaryCols<_> =\n                        rows[width * memory_len..width * memory_len + width].borrow_mut();\n                    forged_row.addr_space = Val::<SC>::from_canonical_u32(forged_as);\n                    forged_row.pointer = Val::<SC>::from_canonical_u32(forged_ptr);\n                    forged_row.initial_data = Val::<SC>::ZERO;\n                    forged_row.final_data = Val::<SC>::ZERO;\n                    forged_row.final_timestamp = Val::<SC>::ZERO;\n                    forged_row.is_valid = Val::<SC>::ONE;\n                }\n                // The anchor row\'s less-than aux must now witness anchor < forged.\n                let mut out = Val::<SC>::ZERO;\n                {\n                    let anchor_row: &mut VolatileBoundaryCols<_> =\n                        rows[width * anchor_idx..width * anchor_idx + width].borrow_mut();\n                    air.addr_lt_air.0.generate_subrow(\n                        (\n                            self.range_checker.as_ref(),\n                            &[\n                                Val::<SC>::from_canonical_u32(anchor_as),\n                                Val::<SC>::from_canonical_u32(anchor_ptr),\n                            ],\n                            &[\n                                Val::<SC>::from_canonical_u32(forged_as),\n                                Val::<SC>::from_canonical_u32(forged_ptr),\n                            ],\n                        ),\n                        ((&mut anchor_row.addr_lt_aux).into(), &mut out),\n                    );\n                }\n                if out == Val::<SC>::ONE {\n                    eprintln!(\n                        "[beak-witness-inject] kind=openvm.semantic.memory.volatile_boundary_range step={} site=volatile_boundary anchor_as={} anchor_ptr={} forged_as={} forged_ptr={}",\n                        anchor_idx,\n                        anchor_as,\n                        anchor_ptr,\n                        forged_as,\n                        forged_ptr\n                    );\n                    fuzzer_utils::mark_witness_mutation_applied(\n                        "openvm.semantic.memory.volatile_boundary_range",\n                        anchor_idx as u64,\n                    );\n                    fuzzer_utils::record_semantic_mutation(\n                        "openvm.semantic.memory.volatile_boundary_range",\n                        "volatile_boundary.generate_air_proof_input",\n                        "pointer",\n                        anchor_idx as u64,\n                        serde_json::json!({"address_space": anchor_as, "pointer": anchor_ptr}),\n                        serde_json::json!({"address_space": forged_as, "pointer": forged_ptr}),\n                        serde_json::json!({"relation": "volatile_boundary_range", "context": {\n                            "bucket_id": "sem.memory.volatile_boundary_range",\n                            "backend": "openvm",\n                            "trace_source": "volatile_boundary",\n                            "mode": "remap_boundary_cell",\n                            "domain": "volatile",\n                            "guard": "outside_range",\n                            "cell_id": remap.cell_id,\n                            "row_idx": anchor_idx,\n                            "row_anchor": anchor_idx,\n                            "appended_row_idx": memory_len,\n                            "address_space": remap.address_space,\n                            "pointer": remap.base_pointer,\n                            "width": remap.width,\n                            "forged_address_space": forged_as,\n                            "forged_pointer": forged_ptr,\n                            "volatile_start": remap.volatile_start,\n                            "volatile_end": remap.volatile_end,\n                            "address_space_before": anchor_as,\n                            "address_space_after": forged_as,\n                            "pointer_before": anchor_ptr,\n                            "pointer_after": forged_ptr,\n                            "forged_address": forged_ptr,\n                            "outside_volatile_range": true,\n                            "final_row_transition_count": 0\n                        }}),\n                    );\n                } else {\n                    eprintln!(\n                        "[beak-witness-inject] kind=openvm.semantic.memory.volatile_boundary_range step={} site=volatile_boundary skip_unsorted anchor_as={} anchor_ptr={} forged_as={} forged_ptr={}",\n                        anchor_idx,\n                        anchor_as,\n                        anchor_ptr,\n                        forged_as,\n                        forged_ptr\n                    );\n                }\n            }\n        }\n        // BEAK-INSERT-END\n'
        if post_anchor in c:
...
def _patch_frozen_time_origin_wrap_witness_injection(openvm_install_path = None, commit_or_branch = None):
    commit = resolve_openvm_commit(commit_or_branch) if commit_or_branch else None
    # The ts1 baseline observation lives in the offline memory initial block on f038,
    # but in instruction-source observations on 336f; the receipt must bind the same source.
    ts1_source = 'memory_initial_block' if commit == OPENVM_BENCHMARK_F038_COMMIT else 'instruction'
...
def _patch_f038_loadstore_mem_as_witness_injection(openvm_install_path = None):
    ...
    if pre_marker in c:
        start = c.rfind('\n', 0, c.index(pre_marker)) + 1
        end_marker = '        // BEAK-INSERT-END\n'
        end = c.index(end_marker, start) + len(end_marker)
        if c[end:end + 1] == '\n':
            end += 1
        c = c[:start] + c[end:]
    # Observation only: the o51 mutation rewrites the instruction `e` operand in the
    # program ROM (build_exe); the adapter just propagates it through the memory ops.
    pre_insert = '        // BEAK-INSERT: guard.f038.loadstore.adapter.mem_as_pre_access\n        // Observation only: the o51 address-space mutation rewrites the instruction\'s\n        // `e` operand in the program ROM (executor level); the adapter propagates it.\n        let beak_mem_as = e;\n        // BEAK-INSERT-END\n\n'
    if pre_marker not in c and '        let read_record = match local_opcode {\n' in c:
        c = c.replace('        let read_record = match local_opcode {\n', pre_insert + '        let read_record = match local_opcode {\n', 1)
    if pre_marker in c:
...
def apply(*, openvm_install_path, commit_or_branch):
    ...
    if commit in {
        ...
        _patch_frozen_time_origin_wrap_witness_injection(openvm_install_path, commit)
