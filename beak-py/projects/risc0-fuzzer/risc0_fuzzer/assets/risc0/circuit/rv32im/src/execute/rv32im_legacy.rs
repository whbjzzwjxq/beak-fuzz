...
const BEAK_EXEC_INJECT_KIND_ENV: &str = "BEAK_RISC0_EXEC_INJECT_KIND";
const BEAK_EXEC_INJECT_STEP_ENV: &str = "BEAK_RISC0_EXEC_INJECT_STEP";
const BEAK_EXEC_INJECT_HIT_ENV: &str = "BEAK_RISC0_EXEC_INJECT_HIT";
const BEAK_EXEC_INJECT_KIND_SOURCE_BINDING_WORD: &str = "source_operand_binding_word";
const BEAK_EXEC_INJECT_KIND_DEST_BINDING_WORD: &str = "dest_binding_word";
const BEAK_EXEC_INJECT_KIND_OP_SELECTOR_WORD: &str = "op_selector_binding_word";
const BEAK_EXEC_INJECT_KIND_CONTROL_FLOW_WORD: &str = "control_flow_binding_word";
const BEAK_EXEC_INJECT_KIND_MEMORY_EFFECT_WORD: &str = "memory_effect_binding_word";
...
impl Emulator {
    ...
    fn set_rd(word: u32, rd: u32) -> u32 {
        (word & !0x0000_0f80) | ((rd & 0x1f) << 7)
    }

    fn set_rs1(word: u32, rs1: u32) -> u32 {
        (word & !0x000f_8000) | ((rs1 & 0x1f) << 15)
    }

    fn set_rs2(word: u32, rs2: u32) -> u32 {
        (word & !0x01f0_0000) | ((rs2 & 0x1f) << 20)
    }

    fn set_funct3(word: u32, funct3: u32) -> u32 {
        (word & !0x0000_7000) | ((funct3 & 0x7) << 12)
    }

    fn record_injection(word: u32, new_word: u32) -> Option<u32> {
        if new_word == word {
            return None;
        }
        env::set_var(BEAK_EXEC_INJECT_HIT_ENV, format!("{:#010x}->{:#010x}", word, new_word));
        Some(new_word)
    }

    fn maybe_inject_word(&mut self, word: u32) -> Option<u32> {
        let kind = env::var(BEAK_EXEC_INJECT_KIND_ENV).ok()?;
        let target_step = env::var(BEAK_EXEC_INJECT_STEP_ENV)
            .ok()?
            .parse::<u64>()
            .ok()?;
        ...
        match kind.as_str() {
            BEAK_EXEC_INJECT_KIND_SOURCE_BINDING_WORD => {
                if matches!(decoded.opcode, 0b0110011 | 0b0100011 | 0b1100011)
                    && decoded.rs1 != decoded.rs2
                {
                    return Self::record_injection(word, Self::set_rs2(word, decoded.rs1));
                }
            }
            BEAK_EXEC_INJECT_KIND_DEST_BINDING_WORD => {
                if matches!(
                    decoded.opcode,
                    0b0110011 | 0b0010011 | 0b0000011 | 0b0110111 | 0b0010111 | 0b1101111
                        | 0b1100111
                ) && decoded.rd != 0
                {
                    let next_rd = if decoded.rd < REG_MAX as u32 {
                        decoded.rd + 1
                    } else {
                        1
                    };
                    return Self::record_injection(word, Self::set_rd(word, next_rd));
                }
            }
            BEAK_EXEC_INJECT_KIND_OP_SELECTOR_WORD => {
                let new_word = match (decoded.opcode, decoded.func3, decoded.func7) {
                    (0b0110011, 0b100, 0b0000001)
                    | (0b0110011, 0b101, 0b0000001)
                    | (0b0110011, 0b110, 0b0000001)
                    | (0b0110011, 0b111, 0b0000001) => Some(Self::set_funct3(word, decoded.func3 ^ 0b010)),
                    (0b0000011, 0b000, _) | (0b0000011, 0b001, _) | (0b0000011, 0b100, _) | (0b0000011, 0b101, _) => {
                        Some(Self::set_funct3(word, decoded.func3 ^ 0b100))
                    }
                    _ => None,
                }?;
                return Self::record_injection(word, new_word);
            }
            BEAK_EXEC_INJECT_KIND_CONTROL_FLOW_WORD => {
                let new_word = match (decoded.opcode, decoded.func3) {
                    (0b1100011, 0b000)
                    | (0b1100011, 0b001)
                    | (0b1100011, 0b100)
                    | (0b1100011, 0b101)
                    | (0b1100011, 0b110)
                    | (0b1100011, 0b111) => Some(Self::set_funct3(word, decoded.func3 ^ 0b001)),
                    (0b1100111, _) if decoded.rs1 != 0 => Some(Self::set_rs1(word, 0)),
                    _ => None,
                }?;
                return Self::record_injection(word, new_word);
            }
            BEAK_EXEC_INJECT_KIND_MEMORY_EFFECT_WORD => {
                let new_word = match decoded.opcode {
                    0b0100011 if decoded.rs1 != decoded.rs2 => Some(Self::set_rs1(word, decoded.rs2)),
                    0b0000011 if decoded.rs1 != 0 => Some(Self::set_rs1(word, 0)),
                    _ => None,
                }?;
                return Self::record_injection(word, new_word);
            }
    ...
    pub fn step<C: EmuContext>(&mut self, ctx: &mut C) -> Result<()> {
        ...
        let mut word = ctx.load_memory(pc.waddr())?;
        if word & 0x03 != 0x03 {
            ctx.trap(Exception::IllegalInstruction(word, 0))?;
            return Ok(());
        }

        if let Some(new_word) = self.maybe_inject_word(word) {
            word = new_word;
        }
