...
fn semantic_routes() -> Vec<SemanticRoute> {
    ...
    vec![
        ...
        semantic_route(
            ...
            "me5.mem_write",
            "memory_access",
            "openvm.semantic.memory.address_space_consistency::mode=rom_e_rewrite,target_as=4",
            ...
            [
                ...
                ("address_space_after", json!(4)),
                ("mode", json!("rom_e_rewrite")),
                ("target_as", json!(4)),
        ...
        semantic_route(
            ...
            "volatile_boundary",
            "openvm.semantic.memory.volatile_boundary_range::mode=remap_boundary_cell,domain=volatile,guard=outside_range,cell_id=rc3.volatile_pointer,row_idx=11,address_space=2,pointer=4095,width=1,forged_address_space=2,forged_pointer=536870912,volatile_start=0,volatile_end=536870912",
            SemanticMutationRelation::VolatileBoundaryRange,
            11,
            "volatile_boundary.generate_air_proof_input",
            "pointer",
            json!({"address_space": 2, "pointer": 4095}),
            json!({"address_space": 2, "pointer": 536870912}),
            [
                ("row_idx", json!(11)),
                ("row_anchor", json!(11)),
                ("mode", json!("remap_boundary_cell")),
                ("domain", json!("volatile")),
                ("guard", json!("outside_range")),
                ("address_space", json!(2)),
                ("pointer", json!(4095)),
                ("width", json!(1)),
                ("forged_address_space", json!(2)),
                ("forged_pointer", json!(536870912)),
                ("volatile_start", json!(0)),
                ("volatile_end", json!(536870912)),
                ("address_space_before", json!(2)),
                ("address_space_after", json!(2)),
                ("pointer_before", json!(4095)),
                ("pointer_after", json!(536870912)),
                ("forged_address", json!(536870912)),
                ("outside_volatile_range", json!(true)),
            ],
            [("row_idx", "row_idx")],
            [],
